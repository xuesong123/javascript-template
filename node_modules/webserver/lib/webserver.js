...
HtmlUtil.replace = function(e){return this.replacement[e];};
...
HtmlUtil.encode = function(source, crlf){
    ...
    return source.replace(new RegExp("[<>\"\\u00ae\\u00a9]", "g"), this.replace).replace(new RegExp("\\r?\\n", "g"), crlf);
};
...
WebServer.prototype.dispatch = function(request, response){
    ...
    try
    {
        ...
        else
        {
            ...
            response.end("<h1 error=\"10001\">Request URL: " + HtmlUtil.encode(request.requestURL) + " not found !</h1>");
        }
    }
    catch(e)
    {
...
WebApplicationFactory.create = function(host, port, home, path){
    var webApplication = new WebApplication();
    var servletContext = ServletContextFactory.create(host, port, home, path);
    ...
    servletContext.defaultServlet = httpd.create(host, port, home, path);
 ...
 * Use is subject to license terms.
 */
function WebApplication(host, port, home, path){
};
...
WebApplication.prototype.getHost = function(){
    if(this.servletContext != null)
    {
        return this.servletContext.getHost();
    }

    return null;
};

WebApplication.prototype.setPort = function(port){
    if(this.servletContext != null)
    {
        this.servletContext.setPort(port);
    }
};

WebApplication.prototype.getPort = function(){
    if(this.servletContext != null)
    {
...
WebApplication.prototype.getHome = function(){
    if(this.servletContext != null)
    {
        return this.servletContext.getHome();
    }
...
WebApplication.prototype.getPath = function(){
    if(this.servletContext != null)
    {
        return this.servletContext.getPath();
    }
...
WebApplication.prototype.getSessionContext = function(){
...
WebApplication.prototype.getResponse = function(request, response){
    ...
    response.print = function(content){
        if(content != null){
...
WebApplication.prototype.dispatch = function(req, res){
    ...
    {
        res.statusCode = 200;
        res.setHeader("Server", "ayada/1.1");
        ...
        if(servletContext.running() == false)
        {
            ...
            res.end("<h1 error=\"10002\">Request URL: " + HtmlUtil.encode(req.url) + " not found !</h1>");
            return;
        }
        ...
        var requestURI = request.requestURI.toUpperCase();

        sessionContext.update(request, response);
        request.setAttribute("servlet_request_dispatcher", "REQUEST");

        if(StringUtil.startsWith(requestURI, "/WEB-INF"))
        {
        ...
        if(StringUtil.startsWith(requestURI, "/META-INF"))
        {
            response.writeHead(403, "Forbidden", {"Content-Type": "text/plain"});
            response.end();
            return;
        ...
        var servletChain = servletContext.getServletChain(url.pathname);
...
WebApplication.prototype.execute = function(request, response, servletChain){
    ...
    else
    {
        ...
        response.end("<h1 error=\"10002\">Request URL: " + HtmlUtil.encode(request.url) + " not found !</h1>");
        return 404;
    }
};
 ...
 * @param path
 */
ServletContextFactory.create = function(host, port, home, path){
    return new ServletContext(host, port, home, path);
};
 ...
 * Use is subject to license terms.
 */
function ServletContext(host, port, home, path){
    this.host = host;
    this.port = port;
    this.home = home;
    this.path = path;
 ...
 * @return String
 */
ServletContext.prototype.getHost = function(){
    return this.host;
};
 ...
 * @param host
 */
ServletContext.prototype.setPort = function(port){
    this.port = port;

    if(this.defaultServlet != null)
    {
        this.defaultServlet.setPort(port);
    }
};
 ...
 * @return String
 */
ServletContext.prototype.getPort = function(){
    return this.port;
};
        ...
        for(var i = 0, length = list.length; i < length; i++)
        {
            ...
            if(stats.isFile())
            {
                if(fileName.length >= 3 && fileName.substring(fileName.length - 3).toLowerCase() == ".js")
                {
                    try
                    {
                        ...
                        if(servlets != null)
                        {
                            for(var name in servlets)
                            {
                                if(map[name] != null)
                                {
                                    throw new Error(filePath + " - servlet \"" + name + "\ already exists!");
                                }
                    ...
                    {
                        LogUtil.info("Error at " + filePath);
                        console.trace();
                        throw e;
                    }
                }
...
ServletContext.prototype.load = function(force){
    ...
    try
    {
        ...
        var contextPath = StringUtil.trim(webConfig.path);
...
HttpServlet.prototype.service = function(request, response, servletChain){
    ...
    else
    {
        ...
        response.end("<h1 error=\"10003\">UNKNOWN METHOD: " + HtmlUtil.encode(method) + "</h1>");
    }
};
...
JspServlet.prototype.execute = function(request, response, servletChain){
    ...
    if(fs.existsSync(file))
    {
        ...
        {
            this.context[uri] = object = null;
        }

        if(object == null)
        {
            var source = fs.readFileSync(file, "UTF-8");
            var servlet = this.compile(source);
    ...
    else
    {
        ...
        response.end("<h1 error=\"10004\">Request URL '" + HtmlUtil.encode(request.requestURI) + "' not found !</h1>");
    }
};

JspServlet.prototype.compile = function(source){
    var servlet = {};
    var code = JspRuntime.compile(source, this.template);
    servlet.execute = (new Function(code))();

    servlet.service = function(request, response, servletChain){
        try
        {
...
JspRuntime.compile = function(source, template){
    var c;
    var code = [];
    var text = [];
    var result = [];
    ...
    for(var i = 0; i < length; i++)
    {
        ...
        if(c == '<' && i < length - 1 && source.charAt(i + 1) == '%')
        {
            for(var j = i + 2; j < length; j++)
            {
                ...
                if(c == '%' && j < length - 1 && source.charAt(j + 1) == '>')
                {
                    i++;
                    break;
                }
            ...
            if(text.length > 0)
            {
                result.push("    out.write(\"" + text.join("") + "\");");
                text.length = 0;
            }
            ...
            if(content.length > 0)
            {
                if(content.charAt(0) == "=")
                {
                    result.push("    out.print(" + content.substring(1) + ");");
                }
                else
                {
                    result.push(content);
    ...
    result.push("    try{");
    result.push("        out.flush();");
    result.push("    }");
    result.push("    catch(e){");
    result.push("    }");
    return template.replace("${METHOD_BODY}", result.join("\r\n"));
};

JspRuntime.getWriter = function(){
    return {
        buffer: [],
...
JspRuntime.quote = function(source){
    if(source == null)
    {
        return "";
    }
...
Bootstrap.create = function(host, port, home){
    var webServer = new WebServer();
    var vistualHost = new VistualHost(host, port);
    ...
    if(stats.isDirectory())
    {
        ...
        for(var i = 0, length = list.length; i < length; i++)
        {
            ...
            if(stats.isDirectory())
            {
                var app = WebApplicationFactory.create(host, port, path.join(root, dir), "/" + dir);
        ...
        if(s1 != null && (s1 = this.trim(s1)).length > 0)
        {
            if(this.startsWith(s1, "-"))
            {
                ...
                if(j < args.length)
                {
                    ...
                    if(s2 != null && (s2 = this.trim(s2)).length > 0)
                    {
